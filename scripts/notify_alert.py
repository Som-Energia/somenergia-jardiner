...
def refresh_notification_table(con, schema, alertdf, alert_name):
    ...
    table_exists = True
    try:
        alert_status_df_old = pd.read_sql_table(
            con=con, table_name=table_name, schema=schema
        )
    except ValueError:
        table_exists = False
    ...
    if table_exists:
        ...
        alertdf_new_clean_merged = alertdf_new_clean.merge(
            alert_status_df_old_clean,
            how="left",
            on=["plant_id", "plant_name", "device_type", "device_name", "alarm_name"],
        )
        alertdf_new_clean["is_alarmed"] = np.where(
            alertdf_new_clean_merged["is_alarmed_x"].isnull(),
            alertdf_new_clean_merged["is_alarmed_y"],
            alertdf_new_clean_merged["is_alarmed_x"],
        )
        # If still NULL then False (rare case without readings 30 days before):
        alertdf_new_clean["is_alarmed"] = np.where(
            alertdf_new_clean["is_alarmed"].isnull(),
            False,
        ...
        df = pd.concat([alert_status_df_old_clean, alertdf_new_clean]).reset_index(
            drop=True
        )
        groupby_df = df.groupby(
            ["plant_id", "plant_name", "device_type", "device_name", "is_alarmed"]
        )
    ...
    alertdf_diff.to_sql(
        con=con,
        name=alert_name + "_historic",
        if_exists="append",
        schema=schema,
        index=False,
    )
    alertdf_new_clean.to_sql(
        con=con, name=table_name, if_exists="replace", schema=schema, index=False
    )
...
def evaluate_and_notify_alarm(
    ...
    alertdf, alertdf_diff = refresh_notification_table(
    ...
    notified = df_notify_topic_if_diff(
        alertdf_diff, novu_base_url, api_key, topic_ids, alert
    )

    notification_topics_df = pd.read_sql_table(
        "plant_topic_association", conn, schema=schema
    )

    plants_by_topic = (
        notification_topics_df.groupby("notification_topic")["plant_id"]
        .apply(list)
        .to_dict()
    )
    ...
    for notification_topic, plants in plants_by_topic.items():
        ...
        sub_notified = df_notify_topic_if_diff(
            sub_alertdf_diff, novu_base_url, api_key, [notification_topic], alert
        )
...
def refresh_alert(
    ...
    dbapi = (
        plantmonitor_db  # pending implement custom function jardiner.utils get_dbapi
    )
    db_engine = sqlalchemy.create_engine(dbapi)
    with db_engine.begin() as conn:
        notified = evaluate_and_notify_alarm(
            conn, novu_base_url, api_key, schema, alert, to_notify
        )
