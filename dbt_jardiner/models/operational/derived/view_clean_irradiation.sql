...
{# this view is a legacy view which #}
 SELECT sir.sensor,
   ...
   FROM {{ source('plantmonitordb', 'irradiationregistry') }} sir
     LEFT JOIN {{ source('plantmonitordb', 'sensor') }} sensor ON sensor.id = sir.sensor
     LEFT JOIN {{ source('plantmonitordb', 'plant') }} plant ON plant.id = sensor.plant
     LEFT JOIN {{ source('plantmonitordb', 'satellite_readings') }} sat ON sat.plant = sensor.plant AND time_bucket('1 hour', sat."time") = time_bucket('1 hour', sir."time")