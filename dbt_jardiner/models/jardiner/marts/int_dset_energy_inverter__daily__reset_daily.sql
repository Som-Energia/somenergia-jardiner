...
inverter_energy_daily_metrics as (
  select
    ...
    max(inverter_energy_kwh)
    - min(inverter_energy_kwh) as diff_inverter_energy_kwh,
    max(inverter_energy_kwh)
    - min(nullif(inverter_energy_kwh, 0)) as diff_nozero_inverter_energy_kwh
...
inverter_energy_daily as (
  ...
  order by day desc, plant_name asc, device_name asc
),
...
--data aprox en que dset transforma correctament aquestes mètriques
where day > '2023-12-15'
order by day desc, plant_name asc, device_name asc
