...
select
  ...
  round(
    (overview.instant_power_plant_kw / overview.nominal_power_kw)::numeric,
    2
  ) as potencia_inst_vs_nominal,
  case
    when overview.day > now() - interval '2 days' then
      round(
        (
          (
            overview.meter_exported_energy_kwh
            - overview.solargis_meter_expected_energy_kwh
          )
          / 1000
        )::numeric,
        2
