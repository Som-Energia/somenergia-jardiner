...
unnested_dset_response as (
  select
    ...
    queried_at,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_id'
    )::text as signal_id,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
    ...
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_frequency'
    )::text as signal_frequency,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_is_virtual'
    )::boolean as signal_is_virtual,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_last_value'
    )::numeric as signal_last_value,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_description'
    )::text as signal_description,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_device_external_id'
    )::text as signal_device_uuid,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_device_external_description'
    )::text as signal_device_type,
    (
      jsonb_array_elements((response)::jsonb #> '{signals}')
      ->> 'signal_external_id'
    )::text as signal_uuid,
