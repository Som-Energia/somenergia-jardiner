...
with meter_registry_hourly_raw as (
  ...
  from {{ ref('raw_meterregistry') }} as mr
    left join {{ ref("raw_plantmonitor_plants") }} as p using (plant_id)
  group by date_trunc('hour', mr.start_hour), p.plant_uuid, mr.plant_id, p.plant_name, p.plant_codename, mr.meter_id, mr.meter_name
)
