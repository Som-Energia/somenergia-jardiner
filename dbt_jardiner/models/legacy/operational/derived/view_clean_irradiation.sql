...
{# this view is a legacy view which #}
 SELECT sir.sensor,
   ...
   FROM {{ source('plantmonitor_legacy', 'irradiationregistry') }} sir
     LEFT JOIN {{ source('plantmonitor_legacy', 'sensor') }} sensor ON sensor.id = sir.sensor
     LEFT JOIN {{ source('plantmonitor_legacy', 'plant') }} plant ON plant.id = sensor.plant
     LEFT JOIN {{ source('plantmonitor_legacy', 'satellite_readings') }} sat ON sat.plant = sensor.plant AND time_bucket('1 hour', sat."time") = time_bucket('1 hour', sir."time")