...
with plant_production_daily as (
  select
    ...
    --ens mengem el cavni d'hora
    round(
      sum(meter_registry_hours_with_readings) / (count(*) * 24) * 100, 2
    ) as p_meter_hours_with_readings,
    --ens mengem el cavni d'hora
    round(
      sum(satellite_hours_with_readings) / (count(*) * 24) * 100, 2
    ) as p_satellite_hours_with_readings,
    sum(
      meter_registry_export_energy_kwh
    ) as meter_registry_export_energy_kwh,
    sum(
      meter_registry_hours_with_energy
    ...
    round(
      avg(satellite_readings_module_temperature_dc_mean), 2
    ...
    sum(
      deviation_exported_vs_satellite_expected_kwh
    ) as deviation_exported_vs_satellite_expected_kwh,
    sum(
