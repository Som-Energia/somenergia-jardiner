...
with combined_meter_satellite as (
  select
    ...
    spine.start_hour between sunrise_real and sunset_real as daylight_real,
    spine.start_hour between sunrise_generous and sunset_generous as daylight_generous,
  ...
  left join {{ ref('int_satellite_readings__hourly') }} as satellite_readings
    on satellite_readings.plant_id = spine.plant_id and satellite_readings.start_hour = spine.start_hour
