...
window_observed as (
  ...
  where
    current_date - interval '1 month' < ts
...
gaps_converted_to_n_missing_samples as (
  select
    *,
    ...
    n_gaps
    * ceil(
      extract(epoch from gap) / extract(epoch from signal_frequency) - 1
    ) as n_missing_samples,
    -- 24*60 are the minutes in a day
    (24 * 60)
    / (extract(epoch from signal_frequency) / 60) as n_samples_per_day
