...
with DAG(dag_id='alert_meter_no_readings_v1', start_date=datetime(2022,11,17), schedule_interval='3-59/5 * * * *', catchup=False, tags=["Plantmonitor", "Jardiner"], default_args=args) as dag:
    ...
    notify_alarms_task = DockerOperator(
        ...
        command='python3 -m scripts.notify_alert "{{ var.value.plantmonitor_db }}"\
                "{{ var.value.novu_url }}" "{{ var.value.novu_api_key }}" "{{ var.value.plantmonitor_db_prod_schema }}" "{{ var.value.plantmonitor_alert_reciver }}" alert_meter_no_readings True',
